...
export class DockerProvider implements IEnvironmentProvider {
  ...
  async getEnvironment(id: string): Promise<Environment | null> {
    try {
      const output = execSync(`docker inspect ${id}`, {
        encoding: "utf-8",
        stdio: "pipe",
      });
  ...
  async listEnvironments(filter?: Record<string, unknown>): Promise<Environment[]> {
    const output = execSync(
      'docker ps -a --filter "name=opendistil-*" --format json',
  ...
  async healthCheck(id: string): Promise<EnvironmentHealthCheck> {
    try {
      execSync(`docker exec ${id} echo "ok"`, { stdio: "pipe" });
